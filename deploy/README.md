...
**3. Image pull.** The manifests use `ghcr.io/sassoftware/sas-mcp-server:1.17.0`.
If the cluster cannot pull from ghcr.io, mirror it first:

```sh
podman pull ghcr.io/sassoftware/sas-mcp-server:1.17.0
podman tag ghcr.io/sassoftware/sas-mcp-server:1.17.0 \
  registry.example.com/library/sas-mcp-server:1.17.0
podman push registry.example.com/library/sas-mcp-server:1.17.0
```
